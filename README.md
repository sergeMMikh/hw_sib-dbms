...
> [!IMPORTANT]
> Все команды нужно выполнять в каталоге, в котором расположен ваш файл `docker-compose.yml`.
...
<details>
  <summary>Список ожидаемых результатов</summary>
...
Что бы не было глюков со старым образом- пересобрал [контейнер под новую версию postgres](docker-compose.yml). `docker system prune -a` тоже решает ворпос :)
...
> [!IMPORTANT]
> Откройте новую консоль, то есть окно терминала, в каталоге с файлом `docker-compose.yml` и все команды выполняйте там:
